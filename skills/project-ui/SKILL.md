...
name: project-ui
description: >
  project UI-specific patterns. For generic patterns, see: typescript, react-19, nextjs-16, tailwind-4.
Trigger: When working inside ui/ on project-specific conventions (shadcn vs HeroUI legacy, folder placement, actions/adapters, shared types/hooks/lib).
license: Apache-2.0
metadata:
  ...
  scope: [root, ui]
  auto_invoke:
    - "Creating/modifying project UI components"
    - "Working on project UI structure (actions/adapters/types/hooks)"
...
- `playwright` - E2E testing (see also `project-test-ui`)

## Tech Stack (Versions)

```
Next.js ^16 | React 19.2.2 | Tailwind 4.1.13 | shadcn/ui
